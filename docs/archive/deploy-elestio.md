...
3. **The following information ready:**
   ...
   - Your custom domain name (e.g., `KoNote.myorganisation.ca`)
...
4. Give your service a name (e.g., "KoNote-web")
5. Click **Create**
...
Your KoNote Web repository includes a `docker-compose.yml` file that defines everything the app needs:
...
| `ALLOWED_HOSTS` | Domain names the app will accept requests for (comma-separated) | `KoNote.myorganisation.ca` |
...
| `AZURE_REDIRECT_URI` | Where Azure redirects after login (e.g., `https://KoNote.myorganisation.ca/auth/callback/`) |

---

## Step 4: Set Up PostgreSQL Databases

KoNote Web uses **two separate PostgreSQL databases**:
- **Main database** (`KoNote`): stores clients, outcomes, notes, and all business data
...
3. Create two PostgreSQL instances:
   - First instance:
     - Name: `db` (or `KoNote-db`)
     - Database: `konote`
     - User: `konote`
     - Password: generate a secure password
   - Second instance:
     - Name: `audit_db` (or `KoNote-audit-db`)
...
4. Select the repository `KoNote-web`
...
Your app will be accessible at an Elest.io subdomain by default (e.g., `KoNote-web-12345.elest.io`). To use your own domain with HTTPS:
...
2. Add your custom domain (e.g., `KoNote.myorganisation.ca`)
...
2. Update `ALLOWED_HOSTS` to include your new domain:
   ```
   KoNote.myorganisation.ca,www.KoNote.myorganisation.ca
   ```
3. If using Azure AD, update `AZURE_REDIRECT_URI`:
   ```
   https://KoNote.myorganisation.ca/auth/callback/
   ```
4. Redeploy the app (Elest.io may do this automatically)

Your app is now accessible at `https://KoNote.myorganisation.ca`.
...
1. **Open the login page:**
   - Go to `https://KoNote.myorganisation.ca/auth/login/`
   - You should see the KoNote login screen
...
3. Test the new version at `https://KoNote.myorganisation.ca`
...
KoNote Web uses Gunicorn (the Python web server) with 3 workers by default. Each worker can handle requests concurrently.
...
2. It should look like: `KoNote.myorganisation.ca,www.KoNote.myorganisation.ca`
...
Congratulations! Your KoNote Web deployment is complete. Here's what you've set up:
...
You can now log in at `https://KoNote.myorganisation.ca` and start using KoNote Web with your organisation.
